...
async def generate_audio(data: TTSRequest):
    ...
    try:
        ...
        # Use SDK generate call — returns object with audio_file attribute
        response = client.text_to_speech.generate(
            text=data.text,
            voice_id="en-US-natalie",  # change if you want
            style="Promo",
            format="MP3"  # explicit format
        )
        # response.audio_file is a public URL (Murf docs)
...
async def transcribe_file(file: UploadFile = File(...)):
    ...
    """
    try:
        # Save to temp file path and pass path to AssemblyAI SDK (recommended)
        with tempfile.NamedTemporaryFile(delete=False, suffix=os.path.splitext(file.filename)[1] or ".webm") as tmp:
            tmp.write(await file.read())
            tmp_path = tmp.name

        transcriber = aai.Transcriber()
        # This call accepts a local file path and returns a Transcript object
...
async def tts_echo(file: UploadFile = File(...)):
    ...
    """
    try:
        # Save uploaded audio to a temp file (AssemblyAI SDK handles local file paths well)
        with tempfile.NamedTemporaryFile(delete=False, suffix=os.path.splitext(file.filename)[1] or ".webm") as tmp:
            tmp.write(await file.read())
            tmp_path = tmp.name
        ...
        # 2) Generate Murf TTS from the transcript (using Murf Python SDK)
        client = murf_client
        response = client.text_to_speech.generate(
            text=text,
            voice_id="en-US-natalie",  # keep same voice as /generate-audio if you want
            style="Promo",
            format="MP3"
        )

        murf_audio_url = response.audio_file  # Murf SDK returns this public URL
